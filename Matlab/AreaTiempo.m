...
sound(y,Fs);
    for filas=1:30000
        ...
        posicion = randperm(VECTORDISRUPTOR,errores); %ARRAY DE errores ELEMENTOS UNICOS DE 1 A 176
...
sound(y,Fs);
d=bi2de(B,'left-msb');
dlmwrite('AreaTiempoInput.dat',B,'delimiter','')
tac=datetime-tic
sound(y,Fs);
%%

errores=0;
fileID = fopen('AreaTiempoOutput.dat', 'r');
A=fscanf(fileID, '%f');
C=transpose(A);
